...
// Lets a freshly-created Auth0/Google account pick hiker vs tour_guide once,
// right after sign-in — the one thing the Google/Auth0 redirect itself has
// no room to ask. Deliberately NOT reusable as a general "change my role"
// endpoint: it refuses to run a second time (accountTypeConfirmed gates
// it), which is what stops this from reopening the exact self-promotion
// hole firestore.rules' users/{uid} lockdown was built to close.
exports.setInitialAccountType = onCall(
  ...
  async (request) => {
    ...
    if (accountType !== "hiker" && accountType !== "tour_guide") {
      throw new HttpsError(
        "invalid-argument",
        "accountType must be 'hiker' or 'tour_guide'.",
      );
    }
    ...
    if (snap.data().accountTypeConfirmed === true) {
      throw new HttpsError(
        "failed-precondition",
        "Account type has already been set for this account.",
      );
    }

    // Matches AuthDatabaseService.signUpUser's convention: choosing
    // tour_guide marks pending-verification intent (false, not null),
    // never grants verified status directly — that still requires
    // scripts/approveGuide.js.
    const guideVerified = accountType === "tour_guide" ? false : null;
    ...
    await admin
      .auth()
      .setCustomUserClaims(uid, { role: accountType, accountType, guideVerified });
